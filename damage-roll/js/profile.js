...
// Cloud sync -- not yet implemented. Set these once Supabase auth is wired up.
let _syncedUid  = null;  // Supabase auth user id (uuid string)
let _syncedCode = null;  // Short display/room code tied to the account

// Placeholder -- replace with a real Supabase profile fetch when auth is ready.
function _fetchProfileByUid(uid, silent = false) {
    // TODO: query the `profiles` table by uid and merge into _profileData
    if (!silent) console.log('[DR] _fetchProfileByUid called, not yet implemented. uid:', uid);
}
...
function _startOnlineBtn(fn) {
    if (!_syncedUid) {
        // Not logged in — don't navigate, just make sure the notice is visible
        _updateStartScreen();
        return;
    }
    playSfx('menuClick');
