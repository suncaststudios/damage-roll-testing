...
function unlockAch(id) {
    ...
    const reward = goldByRarity[def.rarity] || 15;
    if (typeof shopAwardGold === 'function') {
        shopAwardGold(reward);
        setTimeout(() => {
            if (typeof _showGoldToast === 'function') _showGoldToast(`+${reward} 🪙 Achievement: ${def.name}`);
        }, 3600); // after achievement toast fades
    }
}
...
function trackCrit(isPlayer) {
    if (isPlayer) { achStats.totalCrits++; achStats._battleCrits++; }
    checkAchs();
}
...
function trackGameEnd(won) {
    ...
    if (won) {
        ...
        // Award gold for winning (online only for now — offline is practice)
