/* LEADERBOARD SYSTEM  –  Supabase backend
   ---------------------------------------------------------------
   Reads from these Supabase tables (created via the profiles/clubs SQL):
     profiles  — id, username, avatar, rank_score, wins, losses, tournaments_won, club_id
     clubs     — id, name, tag, badge, trophies

   Add these columns to `profiles` if not already present:
     alter table profiles add column if not exists rank_score     int default 0;
...
function switchLbTab(id) {
    _lbState.tab = id;
    const tabs = ['top-rank','most-wins','most-losses','club-rank','tournaments'];
    ...
    try {
        ...
        if (id === 'top-rank') {
            const { data } = await sb.from('profiles')
                .select('id,username,avatar,rank_score')
                .order('rank_score', { ascending: false }).limit(20);
            rows = (data||[]).map((r,i) => _lbRow(i, r.avatar||'\u2694\uFE0F', r.username||'Wanderer',
                null, r.rank_score??0, 'pts', r.id));
