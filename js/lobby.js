...
function _lobbyStartPairing() {
    ...
    pairs.forEach(([p1, p2]) => {
        ...
        // Broadcast the pairing so both clients know
        _lobby.channel?.send({
...
function _lobbyStartDuel(p1, p2, role) {
    ...
    _broadcastChannel.on(move => {
        ...
        if (!state.turn && move.type === 'play') _applyOpponentMove(move);
    });
...
function _lobbyViewProfile(uid) {
    ...
    sb.from('profiles').select('username,avatar,avatar_img,banner_img,bio,quote,rank_score,wins,losses')
        ...
        .then(({ data, error }) => {
            ...
            const score  = data.rank_score || 0;
            const tier   = typeof getRankTier === 'function' ? getRankTier(score) : { label:'Unranked', color:'#888' };
            ...
            c.innerHTML = `
                ...
                <div style="font-size:10px;color:${tier.color||'#c8a460'};letter-spacing:2px;text-transform:uppercase;margin-bottom:10px;">${tier.label||'Unranked'} · ${score} pts</div>
