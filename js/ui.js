...
(function() {
    ...
    window.sendChatMessage = function() {
        ...
        input.value = '';
        input.style.height = '';
        // Opponent always replies with the fixed message after a short delay
        setTimeout(() => {
            addMsg('Opponent', 'super COOL test message 123 @#$', 'opponent');
            const chat = getChatEl();
            const dot  = getDotEl();
            if (chat && !chat.classList.contains('chat-open') && dot) {
